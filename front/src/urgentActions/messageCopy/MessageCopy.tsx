...
import LetterView from '../messageView/LetterView';
...
export const MessageCopy = ({ className, text, messageTemplate, action }: Props) => {
    if (!messageTemplate || !messageTemplate.length) {
        return <p className="error">Cette action urgente n&#39;existe plus.</p>;
    }

    return (
        <div id="jb" className={classnames('message-copy', className)}>
...
const WithStylesMessageCopy = styled(MessageCopy)(styles);
// @ts-expect-error TS(2769): No overload matches this call.
export default compose(withYellowLogo, withSessionData)(WithStylesMessageCopy);
